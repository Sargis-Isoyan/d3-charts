angular.module('app')
    .directive('discountbyplayerChart', function ($rootScope) {
        return {
            restrict: 'EA',
            scope: {
                data: '='
            ...
            link: function (scope, elem, attrs) {
                ...
                // add the SVG element
                var svg = d3.selectAll(".idfordiscaountbyplayerchart")
                    .append("svg")
                ...
                var div1 = d3.select(svg[0][ind]).append("g")
                            .attr("transform",function(d,j){ return  "translate("+(rectNum*300)+",20)"})
                            .style("float","left")
                            .style("width","25%")
              ...
              div1.append("rect")
                    ...
                    div1.append("text")
                        ...
                        .attr("transform",function(d,j){ return  "translate("+(rectNum*20+50)+",-35)"})
                        .style("float","left")
                        .style("width","25%")
                ...
                var chart = div1.append('g')
                    ...
                    .attr({'x':0,'y':function(d,o){ return yscale(o)-2; }})
                    .style('fill',function(d,o){ return colorScale(o); })
                    .attr('width',function(d){ return (d.Share*250)/100; })
                    .on("mouseover", function(d,f){
                        $("."+d.category).parent(".barsfordiscbyplayer").siblings('.info').attr("style","display:block").attr("transform","translate("+($(this).attr("width")/2)+","+(yscale(f)+20)+")");
                        $(".text_category").text("Cat"+d.category);
                ...
                var transitext = d3.selectAll('.barsfordiscbyplayer')
                    ...
                    .enter()
                    .append('text')
                    .attr({'x':function(d) {return (((d.Share*250)/100)-35); },'y':function(d,o){ return yscale(o)+20; }})
                    .text(function(d){ return d.Share+"%"; }).style({'fill':'#fff','font-size':'15px'});
                ...
                for(var i =0; i<scope.data.length; i++){    
                        if(i !== 4){
                            ...
                            // $("#discountbyplayer-product ").append(div3)
                        }
                        else{
                            // var div3 = "<div>aaaa</div>";
                            j=0;
